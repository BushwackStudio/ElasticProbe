...
class Features {
	...
	public function get_feature_groups() {
		$groups = [
			'core-search'         => [
				'label' => esc_html__( 'Core Search', 'elasticprobe' ),
			],
			'live-search'         => [
				'label' => esc_html__( 'Live Search', 'elasticprobe' ),
			],
			'indexing-options'    => [
				'label' => esc_html__( 'Indexing Options', 'elasticprobe' ),
			],
			'woocommerce'         => [
				'label' => esc_html__( 'WooCommerce', 'elasticprobe' ),
			],
			'third-party-plugins' => [
				'label' => esc_html__( 'Third Party Plugins', 'elasticprobe' ),
			],
		];
		/**
		 * Filter available groups.
		 *
		 * @hook eprobe_feature_groups
		 * @since 5.3.0
		 * @param  {array} $groups Current groups
		 * @return {array} New groups
		 */
		return apply_filters( 'eprobe_feature_groups', $groups );
	}
	...
	public function setup_features() {
		...
		foreach ( $this->registered_features as $feature ) {
			 ...
			 *
			 * @since 5.3.0
			 * @hook eprobe_should_setup_feature
			 ...
			 */
			if ( apply_filters( 'eprobe_should_setup_feature', $should_setup, $feature ) ) {
				$feature->setup();
			}
		}
	}
