<?php
/**
 * ElasticProbe admin bar handler
 *
 * @since 5.3.0
...
class AdminBar {
	...
	public function add_admin_bar_status( \WP_Admin_Bar $admin_bar ) {
		...
		$admin_bar->add_menu(
			[
				...
				'title' => '<div id="ep-ab-icon">
					<span class="screen-reader-text">ElasticProbe</span>
				</div>
		...
		$admin_bar->add_menu(
			[
				...
				'title'  => __( 'No calls made to Elasticsearch', 'elasticprobe' ),
			]
		);
	}
	...
	public function update_placeholders() {
		...
		$main_query_status = __( 'No', 'elasticprobe' );
		if ( $has_main_query ) {
			$main_query_status = $is_main_query_success ? __( 'Yes', 'elasticprobe' ) : __( 'Failed', 'elasticprobe' );
		}

		$results               = [];
		$results['main_query'] = sprintf(
			/* translators: %s: Yes, Failed, or No */
			__( 'Main query: %s', 'elasticprobe' ),
			$main_query_status
		);
		$results['total_queries'] = sprintf(
			/* translators: %s: Total queries */
			__( 'Total queries: %s', 'elasticprobe' ),
			count( $queries['filtered'] )
		);
		$results['failed_queries'] = sprintf(
			/* translators: %s: Failed queries */
			__( 'Failed queries: %s', 'elasticprobe' ),
			count( $queries['failed'] )
		);

		// TODO: Clean up
		$results['debugging_article'] = sprintf(
			'<a href="%s">' . __( 'More about debugging', 'elasticprobe' ) . '</a>',
		 ...
		 *
		 * @since 5.3.0
		 * @hook eprobe_admin_bar_status_and_summary
		 ...
		 */
		$filtered_status_and_summary = apply_filters( 'eprobe_admin_bar_status_and_summary', $status_and_summary, $queries, $has_main_query, $is_main_query_success );

		if ( ! isset( $filtered_status_and_summary['status'], $filtered_status_and_summary['summary'] ) ) {
			_doing_it_wrong(
				__METHOD__,
				esc_html__( 'The eprobe_admin_bar_status_and_summary filter must return an array with the status and summary keys.', 'elasticprobe' ),
				'ElasticProbe 1.5.0'
			);
			return;
		}
		 ...
		 *
		 * @since 5.3.0
		 * @hook eprobe_admin_bar_status_and_summary_output
		 ...
		 */
		echo apply_filters( 'eprobe_admin_bar_status_and_summary_output', $final_output, $filtered_status_and_summary, $queries ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	protected function should_display(): bool {
		 ...
		 *
		 * @since 5.3.0
		 * @hook eprobe_admin_bar_should_display
		 * @param {bool} $should_display Whether to display the admin bar status.
		 * @return {bool} New should display value
		 */
		return apply_filters( 'eprobe_admin_bar_should_display', current_user_can( Utils\get_capability( 'admin-bar' ) ) );
	}
