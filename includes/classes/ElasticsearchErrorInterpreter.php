	...
	public function maybe_suggest_solution_for_es( $error ) {
		...
		if ( 'no such index' === $error ) { // ES 5.x error
			return [
				...
				'solution' => sprintf(
					...
					__( 'It seems one of the indices is missing. <a href="%1$s">Delete all data and sync</a> to fix the issue.', 'elasticprobe' ),
					$sync_url
				),
			];
		}
		...
		if ( preg_match( '/no such index \[(.*?)\]/', $error, $matches ) ) {
			return [
				...
				'solution' => sprintf(
					...
					__( 'It seems the %1$s index is missing. <a href="%2$s">Delete all data and sync</a> to fix the issue.', 'elasticprobe' ),
					'<code>' . $matches[1] . '</code>',
					$sync_url
				),
			];
		...
		if ( preg_match( '/No mapping found for \[(.*?)\] in order to sort on/', $error, $matches ) ) {
			return [
				...
				'solution' => sprintf(
					/* translators: 1. Index name; 2. Sync Page URL */
					__( 'The field %1$s was not found. Make sure it is added to the list of indexed fields and run <a href="%2$s">a new sync</a> to fix the issue.', 'elasticprobe' ),
					'<code>' . $matches[1] . '</code>',
					$sync_url
				),
			];
		...
		$field_type_solution = __( 'It seems you saved a post without doing a full sync first because <code>%1$s</code> is missing the correct mapping type. <a href="%2$s">Delete all data and sync</a> to fix the issue.', 'elasticprobe' );
		...
		return [
			...
			'solution' => sprintf(
				...
				__( 'We did not recognize this error. Please consider opening a <a href="%s">GitHub Issue</a> so we can add it to our list of supported errors.', 'elasticprobe' ),
				'https://github.com/BushwackStudio/ElasticProbe/issues/new/choose'
			),
		];
	}
