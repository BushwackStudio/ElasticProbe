...
class Weighting {
	...
	public function setup() {
		 ...
		 *
		 * @hook eprobe_disable_search_weighting
		 * @since 4.0
		 ...
		 */
		if ( apply_filters( 'eprobe_disable_search_weighting', false ) ) {
			return;
		}

		add_action( 'admin_menu', [ $this, 'add_weighting_submenu_page' ], 15 );
		add_filter( 'eprobe_formatted_args', [ $this, 'do_weighting' ], 20, 2 ); // After date decay, etc are injected
		add_filter( 'eprobe_query_weighting_fields', [ $this, 'adjust_weight_for_cross_fields' ], 10, 5 );
	...
	public function get_weightable_fields_for_post_type( $post_type ) {
		...
		$allowed_protected_keys = apply_filters( 'eprobe_prepare_meta_allowed_protected_keys', [], $empty_post );

		sort( $allowed_protected_keys, SORT_STRING );

		/** This filter is documented in includes/classes/Indexable/Post/Post.php */
		$excluded_public_keys = apply_filters( 'eprobe_prepare_meta_excluded_public_keys', [], $empty_post );
		 ...
		 *
		 * @hook eprobe_weighting_fields_for_post_type
		 * @param  {array} $fields Current weighting fields
		 * @param  {string} $post_type Current post type
		 * @return  {array} New fields
		 */
		return apply_filters( 'eprobe_weighting_fields_for_post_type', $fields, $post_type );
	}
	...
	public function get_post_type_default_settings( $post_type ) {
		 ...
		 *
		 * @hook eprobe_weighting_default_enabled_taxonomies
		 * @param  {array}  $enabled_taxonomies Taxonomies that should be enabled by default
		 * @param  {string} $post_type          Post type slug
		 * @return {array}  New taxonomies
		 * @since  3.6.5
		 */
		$enabled_by_default = apply_filters( 'eprobe_weighting_default_enabled_taxonomies', [ 'post_tag', 'category' ], $post_type );
		 ...
		 *
		 * @hook eprobe_weighting_default_post_type_weights
		 * @param  {array} $post_type_defaults Current weighting defaults
		 * @param  {string} $post_type Current post type
		 * @return  {array} New defaults
		 */
		return apply_filters( 'eprobe_weighting_default_post_type_weights', $post_type_defaults, $post_type );
	}
	...
	public function get_weighting_configuration() {
		 ...
		 *
		 * @hook eprobe_weighting_configuration
	...
	public function get_meta_mode() {
		 ...
		 * is changed.
		 *
		 * @hook eprobe_meta_mode
		 * @since 5.0.0
		 * @param string $eprobe_meta_mode Meta mode.
		 * @return string New meta mode.
		 */
		return apply_filters( 'eprobe_meta_mode', 'manual' );
	}
	...
	public function add_weighting_submenu_page() {
		$menu_slug = ( defined( 'EPROBE_IS_NETWORK' ) && EPROBE_IS_NETWORK && ! Utils\is_top_level_admin_context() ) ?
			'elasticprobe' :
	...
	public function update_weighting( $request = null ) {
		 ...
		 *
		 * @since  3.5.x
		 * @hook eprobe_saved_weighting_configuration
		 */
		do_action( 'eprobe_saved_weighting_configuration' );
			...
			foreach ( $fieldset['fields'] as $key => $field ) {
				if ( isset( $weights[ $field ] ) && false !== $weights[ $field ]['enabled'] ) {
					...
					if ( 0 !== $weight ) {
						 ...
						 *
						 * @hook eprobe_query_weighting_fields
	...
	public function post_type_has_fields( $post_type, $args = [] ) {
		 ...
		 *
		 * @hook eprobe_weighting_configuration_for_search
		 * @param  {array} $weight_config Current weight config
		 * @param  {array} $args WP Query arguments
		 * @return  {array} New configuration
		 */
		$weight_config = apply_filters( 'eprobe_weighting_configuration_for_search', $weight_config, $args );
		 ...
		 *
		 * @hook eprobe_weighting_ignore_fields_in_consideration
		 * @param  {array} $fields Current fields
		 * @return  {array} New fields
		 */
		$ignore_keys = apply_filters( 'eprobe_weighting_ignore_fields_in_consideration', [ 'terms.ep_custom_result.name' => true ] );
	...
	public function do_weighting( $formatted_args, $args ) {
		 ...
		 *
		 * @hook eprobe_enable_do_weighting
		 * @since 4.2.2
		 ...
		 */
		if ( apply_filters( 'eprobe_enable_do_weighting', $should_do_weighting, $weight_config, $args, $formatted_args ) ) {
	...
	protected function apply_weighting( $formatted_args, $args, $weight_config ) {
		...
		*/
		...
		foreach ( (array) $args['post_type'] as $post_type ) {
			 ...
			 *
			 * @hook eprobe_weighted_query_for_post_type
			...
			$new_query['bool']['should'][] = apply_filters(
				'eprobe_weighted_query_for_post_type',
				[
					'bool' => [
						'must'   => [
							$current_query,
		 ...
		 *
		 * @hook eprobe_weighting_added
		 * @param  {array} $formatted_args Elasticsearch query
		 * @param  {array} $args WP Query arguments
		 */
		do_action( 'eprobe_weighting_added', $formatted_args, $args );
