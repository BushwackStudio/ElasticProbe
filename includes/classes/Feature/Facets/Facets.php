...
class Facets extends Feature {
	...
	public function is_facetable( $query ) {
		 ...
		 *
		 * @deprecated 5.3.0 Use the 'eprobe_is_facetable' argument in WP_Query instead.
		 *
	...
	public function set_query_aggregations( $query, $aggregations ): void {
		...
		if ( $query->is_main_query() && $this->should_sync_to_global( $query ) ) {
			_doing_it_wrong(
				__METHOD__,
				esc_html__( 'The global variable $GLOBALS[\'ep_facet_aggs\'] is deprecated. Access aggregation data directly from the query object using $query->ep_aggregations or the Facets feature methods get_query_aggregations() and get_facet_aggregation().', 'elasticprobe' ),
				'ElasticProbe 1.5.0'
			);
	...
	protected function should_sync_to_global( $query ) {
		 ...
		 *
		 * @since 5.3.0
		 * @hook eprobe_facet_sync_aggregations_to_global
		 * @param {bool}     $sync  Whether to sync (default: false)
		 * @param {WP_Query} $query The query object
		 * @return {bool} Whether to sync aggregations to global variable
		 */
		return apply_filters( 'eprobe_facet_sync_aggregations_to_global', false, $query );
	}
