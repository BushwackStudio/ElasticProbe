...
class Widget extends \WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'ep-facet-meta',
			esc_html__( 'ElasticProbe - Filter by Metadata', 'elasticprobe' ),
			[
	...
	public function widget( $args, $instance ) {
		...
		$renderer_class = apply_filters( 'eprobe_facet_renderer_class', __NAMESPACE__ . '\\Renderer', 'meta', 'widget', $instance );
	...
	public function form( $instance ) {
		$meta_fields = array_merge(
			[ '' => __( 'Select key', 'elasticprobe' ) ],
			\ElasticProbe\Indexables::factory()->get( 'post' )->get_distinct_meta_field_keys()
		);
		...
		$instance = wp_parse_args(
			...
			[
				...
				'order'             => 'desc',
				'displayCount'      => false,
				'searchPlaceholder' => esc_html__( 'Search', 'elasticprobe' ),
			]
		);

		$orderby_options = [
			[
				'label' => __( 'Highest to lowest count', 'elasticprobe' ),
				'value' => 'count/desc',
			],
			[
				'label' => __( 'Lowest to highest count', 'elasticprobe' ),
				'value' => 'count/asc',
			],
			[
				'label' => _x( 'A → Z', 'label for ordering posts by title in ascending order', 'elasticprobe' ),
				'value' => 'name/asc',
			],
			[
				'label' => _x( 'Z → A', 'label for ordering posts by title in descending order', 'elasticprobe' ),
				'value' => 'name/desc',
			],
		];
		...
		<div class="widget-ep-facet-meta">
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'facet' ) ); ?>">
					<?php esc_html_e( 'Filter by:', 'elasticprobe' ); ?>
				</label>
				...
				<small>
					...
					printf(
						...
						esc_html__( 'This is the list of metadata fields indexed in Elasticsearch. If your desired field does not appear in this list please try to %1$ssync your content%2$s', 'elasticprobe' ),
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'searchPlaceholder' ) ); ?>">
					<?php esc_html_e( 'Search field placeholder:', 'elasticprobe' ); ?>
				</label>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'displayCount' ) ); ?>">
					<?php esc_html_e( 'Display count', 'elasticprobe' ); ?>
				</label>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'orderby_custom' ) ); ?>">
					<?php esc_html_e( 'Order by:', 'elasticprobe' ); ?>
				</label><br>
