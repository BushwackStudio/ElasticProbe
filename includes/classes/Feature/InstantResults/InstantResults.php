...
class InstantResults extends Feature {
	...
	public function requirements_status() {
		...
		if ( Utils\is_epio() ) {
			...
			$status->message[] = wp_kses_post( __( "To use this feature you need to be an <a href='https://elasticprobe.com'>ElasticProbe.com</a> customer or implement a <a href='https://github.com/BushwackStudio/elasticprobe-proxy'>custom proxy</a>.", 'elasticprobe' ) );
		}
	...
	public function maybe_bypass_post_exclusion( $bypass_exclusion_from_search, $query ) {
		_doing_it_wrong(
			__METHOD__,
			esc_html__( 'Use the WP_Query argument `eprobe_skip_search_exclusion`.', 'elasticprobe' ),
			'ElasticProbe 1.5.0'
		);
