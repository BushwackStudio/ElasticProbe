...
class Products {
	...
	public function add_product_attributes_to_weighting( $fields, $post_type ) {
		...
		$fields['attributes']['children'][ $variations_skus_key ] = array(
			...
			'label' => __( 'Variations SKUs', 'elasticprobe' ),
		);
	...
	/**
	 * Integrate ElasticProbe with the WooCommerce Admin Product List.
	 *
	...
	public function maybe_display_notice_about_product_ordering( $notices ) {
		...
		$notices['woocommerce_custom_sort'] = [
			'html'    => sprintf(
				...
				__( 'Due to the number of products in the site, you will need to <a href="%s">resync</a> after applying a custom sort order.', 'elasticprobe' ),
				Utils\get_sync_url()
			),
			'type'    => 'warning',
			'dismiss' => true,
	...
	public function add_weight_settings_search( $settings ) {
		?>
		<label><input name="settings[decaying_enabled]" type="radio" <?php checked( $settings['decaying_enabled'], 'disabled_only_products' ); ?> value="disabled_only_products"><?php esc_html_e( 'Disabled for product only queries', 'elasticprobe' ); ?></label><br>
		<label><input name="settings[decaying_enabled]" type="radio" <?php checked( $settings['decaying_enabled'], 'disabled_includes_products' ); ?> value="disabled_includes_products"><?php esc_html_e( 'Disabled for any query that includes products', 'elasticprobe' ); ?></label>
		<?php
	}
	...
	public function add_weight_settings_search_schema( $settings_schema, $feature_slug ) {
		...
		foreach ( $settings_schema as &$setting_schema ) {
			...
			$setting_schema['options'] = array_merge(
				...
				[
					[
						'label' => __( 'Weight results by date, except for product-only queries', 'elasticprobe' ),
						'value' => 'disabled_only_products',
					],
					[
						'label' => __( 'Weight results by date, except for any query that includes products', 'elasticprobe' ),
