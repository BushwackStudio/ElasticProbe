...
class OrdersAutosuggest {
	...
	public function get_hook_prefix(): string {
		return 'eprobe_woocommerce_order_search';
	}
	...
	protected function get_setting_help_message(): string {
		...
		if ( $available ) {
			/* translators: 1: <a> tag (ElasticProbe.com); 2. </a>; 3: <a> tag (KB article); 4. </a>; */
			$message = __( 'You are directly connected to %1$sElasticProbe.com%2$s! Enable autosuggest for Orders to enhance Dashboard results and quickly find WooCommerce Orders. %3$sLearn More%4$s.', 'elasticprobe' );

			return sprintf(
				wp_kses( $message, 'ep-html' ),
				'<a href="https://elasticprobe.com/" target="_blank">',
				'</a>',
		...
		/* translators: 1: <a> tag (ElasticProbe.com); 2. </a>; 3: <a> tag (KB article); 4. </a>; */
		$message = __( 'Due to the sensitive nature of orders, this autosuggest feature is available only to %1$sElasticProbe.com%2$s customers. %3$sLearn More%4$s.', 'elasticprobe' );
