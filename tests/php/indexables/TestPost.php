...
class TestPost extends BaseTestCase {
	 ...
	 * Get Search feature
	 *
	 * @return ElasticProbe\Feature\Search\
	 */
	protected function get_feature() {
		return ElasticProbe\Features::factory()->get_registered_feature( 'search' );
	}
	...
	public function testPostTermSyncSingleLevel() {
		...
		ElasticProbe\Indexables::factory()->get( 'post' )->index( $post->ID, true );

		$post = ElasticProbe\Indexables::factory()->get( 'post' )->get( $post->ID );
	...
	public function testEmptyMetaKey() {
		...
		$meta_data = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta( $post );
	...
	public function testMetaValueTypes() {
		...
		$intval            = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( 13 );
		$floatval          = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( 13.43 );
		$textval           = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( 'some text' );
		$float_string      = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( '20.000000' );
		$bool_false_val    = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( false );
		$bool_true_val     = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( true );
		$dateval           = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( '2015-01-01' );
		$recognizable_time = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( 'third monday of January 2020' );
		$relative_format   = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_meta_value_types( '+1 year' );
	...
	public function testMetaValueTypeDate() {
		...
		// Invalid dates
		$textval        = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, 'some text' );
		$k20_string     = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, '20.000000' );
		$bool_false_val = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, false );
		$bool_true_val  = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, true );
		...
		// Valid dates
		$intval            = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, time() );
		$floatval          = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, 13.43 );
		$float_string      = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, '20.000001' );
		$dateval           = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, '2015-01-01' );
		$recognizable_time = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, 'third day of January 2020' );
		$relative_format   = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_meta_values( $meta_types, '+1 year' );
	...
	public function testDateQueryFiltersRelation() {
		...
		$date_query = new \ElasticProbe\Indexable\Post\DateQuery(
			[
				'relation' => '',
				[
					'year' => 0,
	...
	public function testDateQueryValidateDateValues() {
		...
		$results = \ElasticProbe\Indexable\Post\DateQuery::simple_es_date_filter(
			[
				'w' => 10,
			]
		);
	...
	public function testDateQueryValidateDateDoingItWrong() {
		...
		$this->setExpectedIncorrectUsage( 'ElasticProbe\Indexable\Post\DateQuery' );
	...
	public function testPostConstructor() {
		...
		$this->assertTrue( is_a( $post->sync_manager, '\ElasticProbe\Indexable\Post\SyncManager' ) );
		$this->assertTrue( is_a( $post->query_integration, '\ElasticProbe\Indexable\Post\QueryIntegration' ) );
	}
	...
	public function testQueryDb() {
		$indexable_post_object = new \ElasticProbe\Indexable\Post\Post();
	...
	public function test_query_db_orderby() {
		...
		$post_indexable = new \ElasticProbe\Indexable\Post\Post();
	...
	public function testPostSyncQueueEPKill() {
		...
		$this->assertNotEmpty( ElasticProbe\Indexables::factory()->get( 'post' )->sync_manager->get_sync_queue() );
	...
	public function testPostSyncQueuePermissions() {
		...
		ElasticProbe\Indexables::factory()->get( 'post' )->sync_manager->action_delete_post( $post_id );
		ElasticProbe\Indexables::factory()->get( 'post' )->sync_manager->index_sync_queue();
		ElasticProbe\Elasticsearch::factory()->refresh_indices();

		// Verify we can still get it from ES.
		$document = ElasticProbe\Indexables::factory()->get( 'post' )->get( $post_id );
	...
	public function testPostPrepareDateTerms() {
		...
		$return_prepare_date_terms = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_terms( $date->format( 'Y-m-d H:i:s' ) );
		...
		$return_prepare_date_terms = ElasticProbe\Indexables::factory()->get( 'post' )->prepare_date_terms( '' );
	...
	public function testParentEditedTerm() {
		...
		$document = ElasticProbe\Indexables::factory()->get( 'post' )->get( $post->ID );
	...
	public function testMetaWithoutValue() {
		...
		// Make sure ElasticProbe returns only 2 posts when meta query is set
		$args  = array(
			'ep_integrate' => true,
			'meta_query'   => array(
				array(
		...
		// Make sure ElasticProbe returns only 2 posts when meta key is set
		$args  = array(
			'ep_integrate' => true,
			'meta_key'     => 'test_key',
		);
	...
	public function testGetSearchAlgorithm() {
		 ...
		 */
		$version_40 = \ElasticProbe\SearchAlgorithms::factory()->get( '4.0' );
	 ...
	 * @since 4.4.0
	 * @group post
	 * @expectedIncorrectUsage ElasticProbe\Indexable\Post\Post::get_distinct_meta_field_keys_db_per_post_type
	 */
	...
	public function testPutMappingThrowsError() {
		...
		ElasticProbe\Elasticsearch::factory()->delete_all_indices();
		$mapping = ElasticProbe\Indexables::factory()->get( 'post' )->put_mapping();
	...
	public function testDeletingThumbnailUpdateRelatedIndexablePost() {
		...
		ElasticProbe\Indexables::factory()->get( 'post' )->index( $product_id, true );
		ElasticProbe\Elasticsearch::factory()->refresh_indices();

		wp_delete_attachment( $thumbnail_id, true );
		$this->assertEquals( '', get_post_meta( $product_id, '_thumbnail_id', true ) );

		$ep_post = ElasticProbe\Indexables::factory()->get( 'post' )->get( $product_id );
	...
	public function test_post_thumbnail_has_all_attributes() {
		...
		$ep_post   = ElasticProbe\Indexables::factory()->get( 'post' )->get( $post_id );
	...
	public function testKillSyncForPasswordProtected() {
		...
		$sync_manager = ElasticProbe\Indexables::factory()->get( 'post' )->sync_manager;
	...
	public function test_get_all_allowed_metas_manual() {
		...
		ElasticProbe\Features::factory()->get_registered_feature( 'woocommerce' )->tear_down();
	...
	public function test_is_post_indexable() {
		$sync_manager = new ElasticProbe\Indexable\Post\SyncManager( 'post' );
	 ...
	 * @since 5.2.0
	 * @group post
	 * @expectedDeprecated ElasticProbe\Indexable\Post\SyncManager::add_admin_bar_status
	 */
	...
	/**
	 * Test the `eprobe_intercept_request` argument.
	 *
	 * @since 5.3.0
	 * @group post
	 */
	...
	public function test_maybe_add_doc_status_to_admin_bar_status() {
		...
		add_filter( 'eprobe_doc_status', $change_doc_status, 10, 0 );
		$this->assertSame(
			[
				'status'  => 'custom',
				'summary' => [
		...
		remove_filter( 'eprobe_doc_status', $change_doc_status );
	...
	public function test_index_with_exception() {
		...
		add_filter( 'eprobe_post_sync_args_post_prepare_meta', [ $this, 'throw_exception' ] );
		$this->post_with_exception = $this->ep_factory->post->create();
		$this->assertFalse( $post_indexable->index( $this->post_with_exception, true ) );
		remove_filter( 'eprobe_post_sync_args_post_prepare_meta', [ $this, 'throw_exception' ] );
	}
