...
class TestAdminBar extends BaseTestCase {
	...
	public function test_add_admin_bar_status() {
		...
		$ep_admin_bar = new ElasticProbe\AdminBar();
		...
		add_filter( 'eprobe_admin_bar_should_display', '__return_false' );
		$admin_bar = new \WP_Admin_Bar();
		$ep_admin_bar->add_admin_bar_status( $admin_bar );
	...
	public function test_update_placeholders() {
		...
		// Not displaying, using the eprobe_admin_bar_should_display filter.
		add_filter( 'eprobe_admin_bar_should_display', '__return_false' );
		ob_start();
		$ep_admin_bar->update_placeholders();
		$output = ob_get_clean();
	...
	/**
	 * Test the `eprobe_admin_bar_status_and_summary` filter.
	 *
	 * @since 5.3.0
	 * @group admin-bar
	 */
	public function test_eprobe_admin_bar_status_and_summary_filter() {
		$ep_admin_bar = new ElasticProbe\AdminBar();
		...
		add_filter( 'eprobe_admin_bar_status_and_summary', $change_status_and_summary, 10, 4 );
	...
	/**
	 * Test the `eprobe_admin_bar_status_and_summary_output` filter.
	 *
	 * @since 5.3.0
	 * @group admin-bar
	 */
	public function test_eprobe_admin_bar_status_and_summary_output_filter() {
		$ep_admin_bar = new ElasticProbe\AdminBar();
		...
		add_filter( 'eprobe_admin_bar_status_and_summary_output', $change_output, 10, 3 );
