...
class TestFeatureActivation extends BaseTestCase {
	...
	/**
	 * Test if feature groups can be changed by the `eprobe_feature_groups` filter
	 *
	 * @since 5.3.0
	 * @group feature-activation
	 */
	public function test_get_feature_groups() {
		...
		add_filter( 'eprobe_feature_groups', $filter );
	...
	/**
	 * Test the `eprobe_should_setup_feature` filter functionality
	 *
	 * @since 5.3.0
	 * @group feature-activation
	 */
	public function test_eprobe_should_setup_feature_filter() {
		$feature = new FeatureTest();
		...
		add_filter( 'eprobe_should_setup_feature', $filter, 10, 2 );

		Features::factory()->setup_features();

		$this->assertTrue( $filter_called, 'eprobe_should_setup_feature filter should be called' );
		$this->assertFalse( $feature->setup_called, 'Feature setup should not be called' );

		// Remove filter and test normal behavior
		remove_filter( 'eprobe_should_setup_feature', $filter );
