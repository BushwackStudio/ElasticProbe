...
class TestFacetTypeMeta extends BaseTestCase {
	...
	public function testGetFilterName() {
		...
		add_filter( 'eprobe_facet_meta_filter_name', $change_filter_name );
	...
	public function testGetFilterType() {
		...
		add_filter( 'eprobe_facet_meta_filter_type', $change_filter_type );
	...
	public function testSetWpQueryAggs() {
		...
		add_filter( 'eprobe_facet_meta_fields', $set_facet_meta_field );
		...
		/**
		 * Test the `eprobe_facet_meta_size` filter
		 */
		$change_meta_bucket_size = function ( $size, $meta_field ) {
			return ( 'new_meta_key_1' === $meta_field ) ? 5 : $size;
		};

		add_filter( 'eprobe_facet_meta_size', $change_meta_bucket_size, 10, 2 );
	...
	public function testGetMetaValues() {
		...
		$ep_remote_request_count = did_action( 'eprobe_remote_request' );

		$meta_values = $facet_type->get_meta_values( 'new_meta_key_1' );
		$this->assertSame( $ep_remote_request_count, did_action( 'eprobe_remote_request' ) );

		/**
		 * Test the `eprobe_facet_meta_custom_meta_values` filter
		 */
		$change_meta_values = function ( $meta_values, $meta_key ) {
			return ( 'new_meta_key_1' === $meta_key ) ? [ '123' ] : $meta_values;
		};
		add_filter( 'eprobe_facet_meta_custom_meta_values', $change_meta_values, 10, 2 );
		...
		remove_filter( 'eprobe_facet_meta_custom_meta_values', $change_meta_values );
		...
		/**
		 * Test the `eprobe_facet_meta_value_max_strlen` filter
		 */
		$change_max_str_len = function ( $length, $meta_key ) {
			return ( 'new_meta_key_2' === $meta_key ) ? 4 : $length;
		};
		add_filter( 'eprobe_facet_meta_value_max_strlen', $change_max_str_len, 10, 2 );
		$meta_values_1 = $facet_type->get_meta_values( 'new_meta_key_1' );
		$meta_values_2 = $facet_type->get_meta_values( 'new_meta_key_2' );
	...
	public function testAddQueryFilters() {
		...
		add_filter( 'eprobe_facet_meta_fields', $allow_field );
	...
	public function testGetSanitizeCallback() {
		...
		add_filter( 'eprobe_facet_default_sanitize_callback', $sanitize_function );
