...
use ElasticProbe\Feature\Facets\Types\PostType\Renderer;
/**
...
class TestFacetTypePostTypeRenderer extends BaseTestCase {
	...
	public function testGetPostTypeValueHtml() {
		...
		/**
		 * Test the `eprobe_facet_post_type_value_label` filter
		 */
		$change_label = function ( $label, $value ) {
			return ( 'page' === $value['value'] ) ? 'Different Label' : $label;
		};
		add_filter( 'eprobe_facet_post_type_value_label', $change_label, 10, 2 );
		...
		remove_filter( 'eprobe_facet_post_type_value_label', $change_label );

		/**
		 * Test the `eprobe_facet_post_type_value_accessible_label` filter
		 */
		$change_accessible_label = function ( $label, $value ) {
			return ( 'page' === $value['value'] ) ? 'Apply filter!' : $label;
		};
		add_filter( 'eprobe_facet_post_type_value_accessible_label', $change_accessible_label, 10, 2 );
		...
		remove_filter( 'eprobe_facet_post_type_value_accessible_label', $change_label );

		/**
		 * Test the `eprobe_facet_post_type_value_html` filter
		 */
		$change_html = function ( $html, $value, $url ) {
			return ( 'https://example.com' === $url ) ? '<p>Completely custom made element</p>' : $html;
		};
		add_filter( 'eprobe_facet_post_type_value_html', $change_html, 10, 3 );
