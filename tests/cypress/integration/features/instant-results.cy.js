...
describe('Instant Results Feature', { tags: '@slow' }, () => {
	 ...
	 */
	it("Can't activate the feature if not in ElasticProbe.com nor using a custom PHP proxy", () => {
		if (isEpIo) {
			return;
		}
	...
	describe('Instant Results Available', () => {
		...
		it('Can activate the feature and sync automatically', () => {
			...
			cy.wpCli('elasticprobe list-features')
				.its('stdout')
		...
		describe('Instant Results activated', () => {
			before(() => {
				cy.wpCli('wp elasticprobe put-search-template', true);
			});
