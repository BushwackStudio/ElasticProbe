...
export function getPluginDir(): string {
	return process.cwd().split('/').pop() ?? 'elasticprobe';
}

export function getPluginSlug(slug: string) {
	return slug === 'elasticprobe' ? getPluginDir() : slug;
}
...
export async function updateFeatures(featureName: string, newValues: any) {
	await wpCliEval(
		`
		$features = get_option( 'eprobe_feature_settings', [] );
...
export async function maybeEnableFeature(featureName: string) {
	await wpCli(`elasticprobe activate-feature ${featureName}`, true);
}
...
export async function maybeDisableFeature(featureName: string) {
	await wpCli(`elasticprobe deactivate-feature ${featureName}`, true);
}
...
export async function refreshIndex(indexable: string) {
	const output = await wpCliEval(
		...
		$output = WP_CLI::runcommand( "elasticprobe request {$index}/_refresh --method=POST", [ 'return' => 'all', 'exit_error' => false ] );
		print_r( $output );
		`,
	);
...
export async function setCustomPostTypes() {
	const output = await wpCliEval(
		...
		WP_CLI::runcommand( "elasticprobe sync --include={$page_id},{$post_id},{$movie_id}", [ 'return' => true ] );
		`,
	);
	if (process.env.PWDEBUG === '1') {
		// eslint-disable-next-line no-console
