...
function get_docker_cid() {
	$docker_cid = get_site_option( 'eprobe_tests_docker_cid', '' );
	if ( ! $docker_cid ) {
		$docker_cid = exec( 'cat /etc/hostname' ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.system_calls_exec
		update_site_option( 'eprobe_tests_docker_cid', $docker_cid );
	}
	return $docker_cid;
}
 ...
 */
function eprobe_tests_delete_all_indices() {
	$docker_cid = get_docker_cid();
	if ( ! $docker_cid ) {
	...
	$response_cat_indices = \ElasticProbe\Elasticsearch::factory()->remote_request( '_cat/indices?format=json' );
	...
	foreach ( $indices_from_cat_indices_api as $index ) {
		...
		\ElasticProbe\Elasticsearch::factory()->delete_index( $index['index'] );
	}
}
WP_CLI::add_command( 'elasticprobe-tests delete-all-indices', 'eprobe_tests_delete_all_indices' );
