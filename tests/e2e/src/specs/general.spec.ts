...
test.describe('WordPress can perform standard ElasticProbe actions', { tag: '@group1' }, () => {
	...
	test('Can see quick setup message after enabling the plugin for the first time', async ({
		...
		await expect
			...
			.toContainText('ElasticProbe is almost ready to go.');
	...
	test('Can see a warning in the dashboard if user activates plugin with an Elasticsearch version before or after min/max requirements', async ({
		...
		await expect
			...
			.toContainText('ElasticProbe may or may not work properly.');
	...
	test('Can see Sync and Settings buttons on Settings Page', async ({ loggedInPage }) => {
		await goToAdminPage(loggedInPage, 'admin.php?page=elasticprobe-settings');
	...
	test('Cannot save settings while a sync is in progress', async ({ loggedInPage }) => {
		...
		await wpCli('eval "delete_option( \'eprobe_index_meta\' );"');

		await goToAdminPage(loggedInPage, 'admin.php?page=elasticprobe');
		await wpCli("eval \"update_option( 'eprobe_index_meta', [ 'indexing' => true ] );\"");
	...
	test('Can see ElasticProbe Last Sync Accordion', async ({ loggedInPage }) => {
