...
test.describe('Facets Feature', { tag: '@group2' }, () => {
	...
	test('Can insert, configure, and use the Filter by Taxonomy block', async ({
		...
		const firstBlock = loggedInPage.locator('.wp-block.wp-block-elasticprobe-facet').first();
		const secondBlock = loggedInPage.locator('.wp-block.wp-block-elasticprobe-facet').last();
		...
		facetResponse = loggedInPage.waitForResponse(
			'/wp-json/wp/v2/block-renderer/elasticprobe/facet*',
		);
		await loggedInPage
		...
		const firstBlockFrontend = loggedInPage.locator('.wp-block-elasticprobe-facet').first();
		const secondBlockFrontend = loggedInPage.locator('.wp-block-elasticprobe-facet').last();
	...
	test('Can insert, configure, use, and transform the legacy Facet widget', async ({
		...
		const legacyWidget = loggedInPage
			.locator('.wp-block-legacy-widget:has-text("ElasticProbe - Filter by Taxonomy")')
			.first();
		...
		await loggedInPage
			.locator(
				'.block-editor-block-switcher__popover .editor-block-list-item-elasticprobe-facet',
			)
			.click();
		...
		const block = loggedInPage.locator('.wp-block-elasticprobe-facet').first();
	...
	test.describe('Filter by Metadata block', () => {
		...
		test('Can insert, configure, and use the Filter by Metadata block', async ({
			...
			const firstBlock = loggedInPage
				.locator('.wp-block.wp-block-elasticprobe-facet-meta')
				.last();
			...
			let facetResponse;
			facetResponse = loggedInPage.waitForResponse(
				'/wp-json/wp/v2/block-renderer/elasticprobe/facet-meta*',
			);
			await loggedInPage
			...
			const featuresResponse = loggedInPage.waitForResponse(
				'/wp-json/elasticprobe/v1/features*',
			);
			...
			await secondBlockFrontend
				...
				.click();
			await expect(
				loggedInPage.locator('.wp-block-elasticprobe-facet a[aria-disabled="true"]'),
			).not.toBeVisible();
			await expect(
	...
	test.describe('Filter by Metadata Range block', () => {
		...
		test('Can insert, configure, and use the Filter by Metadata Range block', async ({
			...
			const block = loggedInPage
				.locator('.wp-block.wp-block-elasticprobe-facet-meta-range')
				.last();
			...
			await loggedInPage.waitForResponse('/wp-json/elasticprobe/v1/meta-keys*');
			await block.locator('select').selectOption('numeric_meta_field');
			await loggedInPage.waitForResponse('/wp-json/elasticprobe/v1/meta-range*');
			...
			await expect(await getBlocksList(loggedInPage)).toContainText('Filter by Metadata');
			await insertBlock(loggedInPage, 'Filter by Metadata');
			await loggedInPage.locator('.wp-block-elasticprobe-facet-meta').last().click();
			await openBlockSettingsSidebar(loggedInPage);
			await loggedInPage
				.locator('.block-editor-block-inspector select')
				.first()
			...
			const blockFrontend = loggedInPage.locator('.wp-block-elasticprobe-facet').first();
			...
			await loggedInPage.locator('.wp-block-elasticprobe-facet .term a').first().click();
			await expect(loggedInPage).toHaveURL(/ep_meta_range_filter_numeric_meta_field_min=7/);
			await expect(loggedInPage).toHaveURL(/ep_meta_range_filter_numeric_meta_field_max=14/);
			await expect(loggedInPage).toHaveURL(
	...
	test.describe('Facet by Post Type', () => {
		test('Can insert, configure, and use the Facet by Post Type block', async ({
			...
			const block = loggedInPage
				.locator('.wp-block.wp-block-elasticprobe-facet-post-type')
				.last();
			...
			let facetResponse = loggedInPage.waitForResponse(
				'/wp-json/wp/v2/block-renderer/elasticprobe/facet-post-type*',
			);
	...
	test.describe('Facet by Date', () => {
		test('Can insert, configure, and use the Facet by Date block', async ({ loggedInPage }) => {
			...
			const block = loggedInPage.locator('.wp-block.wp-block-elasticprobe-facet-date').last();
			...
			const blockFrontend = loggedInPage.locator('.wp-block-elasticprobe-facet-date').first();
			...
			const facetResponse = loggedInPage.waitForResponse(
				'/wp-json/wp/v2/block-renderer/elasticprobe/facet-date*',
			);
			await loggedInPage
