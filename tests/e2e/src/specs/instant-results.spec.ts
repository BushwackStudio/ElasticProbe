...
/**
 * Test suite for the Instant Results Feature in ElasticProbe.
 *
...
test.describe('Instant Results Feature', { tag: '@group1' }, () => {
	const instantResultRequestPromise = (page: Page, urlPart: string) => {
		return page.waitForResponse((response) => {
			const requestId = response.request().headers()['x-elasticprobe-request-id'];
	...
	/**
	 * Test that the feature cannot be activated when not in ElasticProbe.com nor using a custom PHP proxy.
	 */
	test("Can't activate the feature if not in ElasticProbe.com nor using a custom PHP proxy", async ({
		loggedInPage,
	}) => {
		// Skip if running on ElasticProbe.com
		if (isEpIo()) {
			test.skip();
		}
	...
	test.describe('Instant Results Available', () => {
		...
		test('Can activate the feature and sync automatically', async ({ loggedInPage }) => {
			...
			const result = await wpCli('elasticprobe list-features');
		...
		test.describe('Instant Results activated', () => {
			...
			test('Can see instant results elements, URL changes, reload, and update after changing search term', async ({
				...
				await wpCli('elasticprobe put-search-template', true);
...
<!-- /wp:buttons -->`,
			...
			test('Is possible to filter the taxonomy terms', async ({ loggedInPage }) => {
				...
				await wpCliEval(
					`
					$output = WP_CLI::runcommand( 'elasticprobe list-features', [ 'return' => 'all', 'exit_error' => false ] );
					print_r( $output );
