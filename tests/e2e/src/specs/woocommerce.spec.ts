...
/**
 * Test suite for WooCommerce feature functionality in ElasticProbe.
 *
 * @module WooCommerce
 */
test.describe('WooCommerce Feature', { tag: '@group2' }, () => {
	...
	test('Can Search Product by Variation SKU', async ({ loggedInPage }) => {
		...
		await wpCli('elasticprobe sync --setup --yes');
	...
	test.describe('Dashboard', () => {
		...
		test('Can search orders from ElasticProbe in WP Dashboard', async ({ loggedInPage }) => {
			await goToAdminPage(loggedInPage, 'edit.php?post_type=shop_order');
