...
test.describe('WP-CLI Commands', { tag: '@group2' }, () => {
	...
	test.describe('wp elasticprobe sync', () => {
		test('Can index all the posts of the current blog', async ({ loggedInPage }) => {
			const result = await wpCli('wp elasticprobe sync');
		...
		test('Can clear the index in Elasticsearch, put the mapping again and then index all the posts if user specifies --setup argument', async ({
			...
			const syncResult = await wpCli('wp elasticprobe sync --setup --yes');
			...
			const statsResult = await wpCli('wp elasticprobe stats');
		...
		test('Can process that many posts in bulk index per round if user specifies --per-page parameter', async () => {
			const result = await wpCli('wp elasticprobe sync --per-page=20');
		...
		test('Can index one post at a time if user specifies --nobulk parameter', async () => {
			const result = await wpCli('wp elasticprobe sync --nobulk');
			const output = result.toString();
			expect(output).toContain('Indexing posts');
		...
		test('Can skip X posts and index the remaining if user specifies --offset parameter', async () => {
			const result = await wpCli('wp elasticprobe sync --offset=10');
			const output = result.toString();
			expect(output).toContain('Indexing posts');
		...
		test('Can index all the posts of a type if user specify --post-type parameter', async () => {
			const postTypeResult = await wpCli('wp elasticprobe sync --post-type=post');
			...
			const totalResult = await wpCli('wp elasticprobe sync');
		...
		test('Can index without using dynamic bulk requests if user specifies --static-bulk parameter', async ({
			...
			const result = await wpCli('wp elasticprobe sync --static-bulk');
			const output = result.toString();
		...
		test('Can stop ongoing syncs with the --force flag', async () => {
			...
			const result = await wpCli('wp elasticprobe sync --force --yes');
			expect(result.toString()).toContain('Sync cleared');
		});
	});

	test('Can delete the index of current blog if user runs wp elasticprobe delete-index', async ({
		loggedInPage,
	}) => {
		const deleteResult = await wpCli('wp elasticprobe delete-index --yes');
		expect(deleteResult.toString()).toContain('Index deleted');

		const statsResult = await wpCli('wp elasticprobe stats', true);
		expect(statsResult.toString()).toContain('is not currently indexed');

		await goToAdminPage(loggedInPage, 'admin.php?page=elasticprobe-health');
		await expect(loggedInPage.locator('.wrap')).toContainText(
			'We could not find any data for your Elasticsearch indices.',
		);
	});

	test('Can put and get mapping of the current blog if user runs wp elasticprobe put-mapping', async () => {
		const result = await wpCli('wp elasticprobe put-mapping');
		const output = result.toString();
		expect(output).toContain('Adding post mapping');
		expect(output).toContain('Mapping sent');

		const getMappingResult = await wpCli('wp elasticprobe get-mapping');
		expect(getMappingResult.toString()).toContain('mapping_version');

		const networkResult = await wpCli('wp elasticprobe put-mapping --network-wide');
	...
	test('Can recreate the alias index which points to every index in the network if user runs wp elasticprobe recreate-network-alias command', async () => {
		// This test is intentionally empty in the original Cypress test
	});

	test('Can throw an error while running wp elasticprobe recreate-network-alias if the plugin is not network activated', async () => {
		const result = await wpCli('wp elasticprobe recreate-network-alias', true);
		expect(result.toString()).toContain('ElasticProbe is not network activated');
	});

	test('Can activate and deactivate a feature', async () => {
		await setDefaultFeatureSettings();

		const alreadyActiveResult = await wpCli('wp elasticprobe activate-feature search', true);
		expect(alreadyActiveResult.toString()).toContain('This feature is already active');

		const deactivateResult = await wpCli('wp elasticprobe deactivate-feature search');
		expect(deactivateResult.toString()).toContain('Feature deactivated');

		const notActiveResult = await wpCli('wp elasticprobe deactivate-feature search', true);
		expect(notActiveResult.toString()).toContain('Feature is not active');

		const activateResult = await wpCli('wp elasticprobe activate-feature search');
		expect(activateResult.toString()).toContain('Feature activated');

		const invalidResult = await wpCli('wp elasticprobe activate-feature invalid', true);
		expect(invalidResult.toString()).toContain('No feature with that slug is registered');

		await wpCli('plugin deactivate woocommerce');
		const woocommerceResult = await wpCli('wp elasticprobe activate-feature woocommerce', true);
		...
		await wpCli('wp elasticprobe deactivate-feature protected_content');
		const protectedContentResult = await wpCli(
			'wp elasticprobe activate-feature protected_content',
			true,
		);
	...
	test('Can list all the active features if user runs wp elasticprobe list-features command', async () => {
		const result = await wpCli('wp elasticprobe list-features');
		expect(result.toString()).toContain('Active features');
	});

	test('Can list all the registered features if user runs wp elasticprobe list-features --all command', async () => {
		const result = await wpCli('wp elasticprobe list-features --all');
		expect(result.toString()).toContain('Registered features');
	});

	test('Can return a string indicating the index is not running', async () => {
		const result = await wpCli('wp elasticprobe get-ongoing-sync-status');
		expect(result.toString()).toContain(
			'{"indexing":false,"method":"none","items_indexed":0,"total_items":-1}',
		);
	});

	test('Can return a string indicating with the appropriate fields if user runs wp elasticprobe get-last-cli-sync command', async () => {
		await wpCli('wp elasticprobe sync');

		const result = await wpCli('wp elasticprobe get-last-cli-sync --clear');
		expect(result.toString()).toContain('"total_time"');

		const clearResult = await wpCli('wp elasticprobe get-last-cli-sync --clear');
		expect(clearResult.toString()).toContain('[]');
	});

	test.describe('multisite parameters', () => {
		const pluginSlug = getPluginSlug('elasticprobe');

		test.beforeAll(async () => {
			await wpCli(`plugin activate ${pluginSlug} --network`);
			const indicesResult = await wpCli('elasticprobe get-indices');
		...
		test('Can index all blogs in network if user specifies --network-wide argument', async ({
			...
			const result = await wpCli('wp elasticprobe sync --network-wide');
			const output = result.toString();
		...
		test('Can index only site in the --url parameter if user does not specify --network-wide argument', async ({
			...
			const result = await wpCli(`wp elasticprobe sync --url=${baseUrl}/second-site`);
			const output = result.toString();
		...
		test('Can delete all the indices and put mappings for the entire network-wide', async ({
			...
			const deleteResult = await wpCli('wp elasticprobe delete-index --network-wide --yes');
			...
			await goToAdminPage(loggedInPage, 'network/admin.php?page=elasticprobe-health');
			await expect(loggedInPage.locator('.wrap')).toContainText(
				'We could not find any data for your Elasticsearch indices.',
			);

			const mappingResult = await wpCli('wp elasticprobe put-mapping --network-wide');
	...
	test('Can set the algorithm version', async () => {
		const defaultResult = await wpCli('wp elasticprobe set-algorithm-version --default');
		expect(defaultResult.toString()).toContain('Done');

		const getDefaultResult = await wpCli('wp elasticprobe get-algorithm-version');
		expect(getDefaultResult.toString()).toContain('default');

		const setVersionResult = await wpCli(
			'wp elasticprobe set-algorithm-version --version=1.0.0',
		);
		expect(setVersionResult.toString()).toContain('Done');

		const getVersionResult = await wpCli('wp elasticprobe get-algorithm-version');
		expect(getVersionResult.toString()).toContain('1.0.0');

		const errorResult = await wpCli('wp elasticprobe set-algorithm-version', true);
	...
	test('Can get the cluster indices information', async () => {
		const result = await wpCli('wp elasticprobe get-cluster-indices');
		expect(result.toString()).toContain('health');
	});

	test('Can get the indices names', async () => {
		const result = await wpCli('wp elasticprobe get-indices');
		expect(result).toBeDefined();

		const prettyResult = await wpCli('wp elasticprobe get-indices --pretty');
	...
	test('Can stop the sync operation and clear it', async () => {
		...
		const stopResult = await wpCli('elasticprobe stop-sync', true);
		...
		const stopAfterMockResult = await wpCli('wp elasticprobe stop-sync');
		expect(stopAfterMockResult.toString()).toContain('Done');

		const clearResult = await wpCli('wp elasticprobe clear-sync');
		expect(clearResult.toString()).toContain('Sync cleared');
	});

	test('can send an HTTP request to Elasticsearch', async () => {
		const result = await wpCli('wp elasticprobe request _cat/indices');
		expect(result).toBeDefined();

		// Check if it throws an error if non-supported method is used
		const postResult = await wpCli('wp elasticprobe request _cat/indices --method=POST');
		...
		const debugResult = await wpCli(
			'wp elasticprobe request _cat/indices --debug-http-request',
		);
