...
test.describe('Dashboard Sync', { tag: '@group2' }, () => {
	...
	test.afterEach(async ({ loggedInPage }) => {
		if (test.info().status === 'failed') {
			await deactivatePlugin(loggedInPage, 'elasticprobe', 'wpCli', 'network');
			await activatePlugin(loggedInPage, 'elasticprobe', 'wpCli', 'singleSite');
			await wpCli('wp elasticprobe clear-sync');
		}
	});
	...
	test('Should only display a single sync option for the initial sync', async ({
		...
		await wpCli('elasticprobe settings-reset --yes');
		...
		await goToAdminPage(loggedInPage, '/admin.php?page=elasticprobe-sync');
	...
	test('Can sync via Dashboard when activated in single site', async ({ loggedInPage }) => {
		await wpCli('wp elasticprobe delete-index --yes');
	...
	test('Can sync via Dashboard when activated in multisite', async ({ loggedInPage }) => {
		await activatePlugin(loggedInPage, 'elasticprobe', 'wpCli', 'network');

		// Sync and remove, so EP doesn't think it is a fresh install
		await wpCli('wp elasticprobe sync --setup --yes');
		await wpCli('wp elasticprobe delete-index --yes --network-wide');
		...
		await goToAdminPage(loggedInPage, 'network/admin.php?page=elasticprobe-sync');
		await loggedInPage.getByRole('button', { name: 'Start sync' }).click();
		await expect(loggedInPage.locator('.ep-sync-progress strong')).toContainText(
			'Sync complete',
			{
		...
		const wpCliResponse = await wpCli('elasticprobe get-indices');
	...
	test('Can pause the dashboard sync, can not activate a feature during sync nor perform a sync via WP-CLI', async ({
		...
		const responsePromise = loggedInPage.waitForResponse('**/wp-json/elasticprobe/v1/sync*');
		await loggedInPage.getByRole('button', { name: 'Start sync' }).click();
		...
		const wpCliResponse = await wpCli('wp elasticprobe sync', true);
	...
	test('Should only display a single sync option if index is deleted', async ({
		...
		// Send mapping
		await wpCli('wp elasticprobe put-mapping');
	...
	test('Should display a list of error types when errors occur during sync', async ({
		...
		const responsePromise = loggedInPage.waitForResponse(
			(response) => {
				return (
					response.url().includes('/wp-json/elasticprobe/v1/sync') &&
