...
test.describe('Documents Feature', { tag: '@group1' }, () => {
	async function enableDocumentsFeature(page: Page) {
		await goToAdminPage(page, 'admin.php?page=elasticprobe');

		// Wait for the API request to complete
		const responsePromise = page.waitForResponse('**/wp-json/elasticprobe/v1/features*');
	...
	test('Can search .pdf', async ({ loggedInPage }) => {
		...
		await wpCli('elasticprobe sync --setup --yes --show-errors');
